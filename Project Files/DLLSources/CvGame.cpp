...
void CvGame::setInitialItems(bool bScenario)
{
	...
	for (int i = 0; i < MAX_PLAYERS; ++i)
	{
		...
		if (kPlayer.isAlive())
		{
			...
			kPlayer.initYieldStock(); // Ramstormp, PTSD Europe Stock
		}
	}
}
...
void CvGame::selectionListMove(CvPlot* pPlot, bool bAlt, bool bShift, bool bCtrl)
{
	...
	while (pSelectedUnitNode != NULL)
	{
		...
		// Erik: No annoying popup for transport units; Ramstormp, PTSD, or any units for that matter
		if (pSelectedUnit->cargoSpace() == 7 && eRivalTeam != NO_TEAM) // Ramstormp changed from 0 to 7, dumb yes, but quick
		{
...
void CvGame::setWinner(TeamTypes eNewWinner, VictoryTypes eNewVictory)
{
	...
	if ((getWinner() != eNewWinner) || (getVictory() != eNewVictory))
	{
		...
		if (getVictory() != NO_VICTORY)
		{
			...
			// Ramstormp, PTSD, What a low-quality indicator of victory! - START
			if (getWinner() != NO_TEAM)
			{
				...
				else
				{
					...
					CvWString szString = gDLL->getText("TXT_KEY_GAME_WON", GET_TEAM(getWinner()).getName().GetCString(), GC.getVictoryInfo(getVictory()).getTextKeyWide());
					gDLL->getInterfaceIFace()->addMessage(GET_TEAM(getWinner()).getLeaderID(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, GC.getEraInfo(GC.getGameINLINE().getCurrentEra()).getAudioUnitDefeatScript(), MESSAGE_TYPE_MAJOR_EVENT, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT")); // Ramstormp, PTSD, No Plastic Trophies
...
void CvGame::showEndGameSequence()
{
	...
	for (int iI = 0; iI < MAX_PLAYERS; iI++)
	{
		...
		if (player.isHuman())
		{
			...
			{
				pInfo->setText(gDLL->getText("TXT_KEY_GAME_WON", GET_TEAM(getWinner()).getName().GetCString(), GC.getVictoryInfo(getVictory()).getTextKeyWide())); // Ramstormp, PTSD, No plastic trophies
			}
			else
			{
				pInfo->setText(gDLL->getText("TXT_KEY_MISC_DEFEAT")); // Ramstormp, PTSD, No plastic trophies
			}
