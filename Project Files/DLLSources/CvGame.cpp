...
void CvGame::setInitialItems(bool bScenario)
{
	...
	for (int i = 0; i < MAX_PLAYERS; ++i)
	{
		...
		if (kPlayer.isAlive())
		{
			...
			kPlayer.setMaxYieldTradeAmount(getCargoYieldCapacity()); // Ramstormp, PTSD, Trade in Smaller Chunks
		}
	}
...
void CvGame::selectionListMove(CvPlot* pPlot, bool bAlt, bool bShift, bool bCtrl)
{
	...
	while (pSelectedUnitNode != NULL)
	{
		...
		// Erik: No annoying popup for transport units - Ramstormp: No annoying popups whatsoever 
		// WTP, ray, unless it is a "Troop only" ship
		//if (pSelectedUnit->cargoSpace() == 0 && eRivalTeam != NO_TEAM)
		if (!GET_PLAYER(getActivePlayer()).isOption(PLAYEROPTION_MOVEMENT_NEVER_MEANS_WAR) && pSelectedUnit != NULL && eRivalTeam != NO_TEAM && (pSelectedUnit->cargoSpace() == 0 || pSelectedUnit->getUnitInfo().isTroopShip()))
		{
...
//				szBuffer = gDLL->getText("TXT_KEY_GAME_WON", GET_TEAM(getWinner()).getName().GetCString(), GC.getVictoryInfo(getVictory()).getTextKeyWide());
			...
			else
			{
				...
				gDLL->UI().addPlayerMessage(GET_TEAM(getWinner()).getLeaderID(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, GC.getEraInfo(GC.getGameINLINE().getCurrentEra()).getAudioUnitDefeatScript(), MESSAGE_TYPE_MAJOR_EVENT, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT")); // Ramstormp, PTSD, No Plastic Trophies
