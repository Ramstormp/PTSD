...
void CvCity::init(int iID, PlayerTypes eOwner, Coordinates initCoord, bool bBumpUnits)
{
	...
	for (int iI = 0; iI < NUM_DIRECTION_TYPES; iI++)
	{
		...
		if (GC.getGameINLINE().isOption(GAMEOPTION_4_PLOT_BORDERS) && iI % 2 == 0 || !GC.getGameINLINE().isOption(GAMEOPTION_4_PLOT_BORDERS))
		{
...
{
	if (GC.getGameINLINE().isOption(GAMEOPTION_4_PLOT_BORDERS) )//&& getPreviousOwner() != NO_PLAYER)
	{
...
int CvCity::getCultureRate() const
{
	...
	// Ramstormp, PTSD, no culture from population
...
void CvCity::doGrowth()
{
	...
	else if (getFood() < 0)
	{
		...
		if (isHuman())
		{
			iDyingLot = GC.getGameINLINE().getSorenRandNum((aPopulationUnits.size() + 2), "Choose starving soul");
			...
			if (iDyingLot < aPopulationUnits.size() && pTargetUnit != NULL)
			{
				...
				gDLL->UI().addPlayerMessage(getOwnerINLINE(), true, GC.getEVENT_MESSAGE_TIME(), szString, GC.getEraInfo(GC.getGameINLINE().getCurrentEra()).getAudioUnitDefeatScript(), MESSAGE_TYPE_MINOR_EVENT, pTargetUnit->getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_RED"), getX_INLINE(), getY_INLINE(), true, true);
	...
	for (YieldTypes eYield = FIRST_YIELD; eYield < NUM_YIELD_TYPES; ++eYield)
	{
		...
		default:
			...
			if (GC.getYieldInfo(eYield).isCargo() && !GC.getYieldInfo(eYield).isIgnoredForStorageCapacity())
			{
				...
				if (iExcess > 0 && getYieldRate(eYield) > 0) // Ramstormp, PTSD, No killing horses for too much cotton
				{
...
int CvCity::getHappinessFromDomesticDemandsFulfilled() const
{
	...
	for (int iYield = 0; iYield < NUM_YIELD_TYPES; ++iYield)
	{
		...
		if (GC.getYieldInfo(eYield).isCargo())
		{
			...
			// Ramstormp, NHPT, Extra happiness for completely fulfilling demands
			if (getYieldDemand(eYield) > 1 && getYieldDemand(eYield) <= getYieldStored(eYield))
			{
				iHapDomesticDemand++;
			}
		}
	...
	for (int iI = 0; iI < MAX_TEAMS; iI++)
	{
		if (GET_TEAM((TeamTypes)iI).isAlive())
		{
			...
			if (iI != GET_TEAM(getTeam()).getID())
			{
				...
				// Ramstormp, NHPT, Only 1 happy max for open borders
