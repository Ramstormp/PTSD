...
void CvCity::init(int iID, PlayerTypes eOwner, int iX, int iY, bool bBumpUnits)
{
	...
	if (!isHuman()) // R&R, ray, code improvement // Ramstormp, PTSD, was isNative()
	{
	...
	for (int iI = 0; iI < NUM_DIRECTION_TYPES; iI ++) // Ramstormp, PTSD, Plus-shaped borders was iI++
	{
		...
		else
		{
			if (pAdjacentPlot != NULL)
			{
				if (pAdjacentPlot->getCulture(getOwnerINLINE()) < GC.getDefineINT("FREE_CITY_ADJACENT_CULTURE") && !isHuman())
				{
					pAdjacentPlot->setCulture(getOwnerINLINE(), GC.getDefineINT("FREE_CITY_ADJACENT_CULTURE"), bBumpUnits);
				}
				else
				{
					pAdjacentPlot->setCulture(getOwnerINLINE(), 0, !bBumpUnits);
				}
				pAdjacentPlot->updateCulture(!bBumpUnits);
			}
		}
	}
/*	for (int iI = 1; iI < NUM_DIRECTION_TYPES; iI += 2)
	{
		pAdjacentPlot = plotDirection(getX_INLINE(), getY_INLINE(), ((DirectionTypes)iI));

		if (pAdjacentPlot != NULL)
		{
			if (pAdjacentPlot->getCulture(getOwnerINLINE()) < GC.getDefineINT("FREE_CITY_ADJACENT_CULTURE") && isNative())
			{
				pAdjacentPlot->setCulture(getOwnerINLINE(), GC.getDefineINT("FREE_CITY_ADJACENT_CULTURE"), bBumpUnits);
			}
			else
...
bool CvCity::canConstruct(BuildingTypes eBuilding, bool bContinue, bool bTestVisible, bool bIgnoreCost) const
{
	...
	// Ramstormp, PTSD, only build culture victory buildings with cultural victory enabled - START
//if (kBuilding.isNationalWonder() && kBuilding.getMaxWorkers() < 4)
//{
//	if (!GC.getGameINLINE().isOption(GAMEOPTION_ONLY_ONE_COLONIST_PER_VILLAGE))
//	{
//		return false;
//	}
//}
...
bool CvCity::canHurry(HurryTypes eHurry, bool bTestVisible) const
{
	...
	if (getProduction() >= getProductionNeeded(YIELD_HAMMERS))
	{
		...
		// Ramstormp, PTSD, No hurry without needed yields - START
		for (YieldTypes eYield = FIRST_YIELD; eYield < NUM_YIELD_TYPES; ++eYield)
		{
			if (GC.getYieldInfo(eYield).isCargo())
			{
...
int CvCity::foodConsumption(int iExtra) const
{
	...
	if (isHuman())
	{
		return ((getPopulation()) * GC.getFOOD_CONSUMPTION_PER_POPULATION() + plot()->getNumDefenders(getOwnerINLINE()) * GC.getFOOD_CONSUMPTION_PER_CITY_DEFENDER());
	}
	// Ramstormp - END
...
{
	return static_cast<CultureLevelTypes>(std::max(0, plusShapedDistance(0, 0, iDX, iDY))); //Ramstormp, PTSD, plus-shaped borders
}
...
int CvCity::getAbandonTimer() const
{
	return m_iAbandonTimer;
}


bool CvCity::isAbandoned() const
{
	return (getAbandonTimer() > 0);
}


void CvCity::setAbandonTimer(int iNewValue)
{
	//bool bOldOccupation;

	if (getAbandonTimer() != iNewValue)
	{
		//bOldOccupation = isAbandoned();

		m_iAbandonTimer = iNewValue;
		FAssert(getAbandonTimer() >= 0);

		//if (bOldOccupation != isAbandoned())
		//{
			//updateCultureLevel();
...
void CvCity::changeAbandonTimer(int iChange)
{
	setAbandonTimer(getAbandonTimer() + iChange);
}
// Ramstormp - END
	...
	//if (!isOccupation()) // R&R mod, vetiarvind, bug fix for units "disappearing" during disorder
	{
		for (int iI = (GC.getNumCultureLevelInfos() - 1); iI > 0; iI--)
		{
			if (getCulture(getOwnerINLINE()) >= GC.getGameINLINE().getCultureLevelThreshold((CultureLevelTypes) iI, getOwnerINLINE()))
			{
				...
				// Ramstormp, PTSD, Zoom until Culture is ready - START
			//	if (eCultureLevel >= 2)
			//	{
...
int CvCity::getRawYieldConsumed(YieldTypes eYieldType) const
{
	...
	if (eYieldType == YIELD_FOOD)
	{
		if (isHuman())
		{
			iYieldConsumed = getPopulation() * GC.getFOOD_CONSUMPTION_PER_POPULATION() + plot()->getNumDefenders(getOwnerINLINE()) * GC.getFOOD_CONSUMPTION_PER_CITY_DEFENDER(); // Ramstormp, PTSD, Guards eat food too
		}
		else
		{
			iYieldConsumed = getPopulation() * GC.getFOOD_CONSUMPTION_PER_POPULATION(); // Ramstormp, PTSD, Guards eat food too
		}
	}
...
// Ramstormp - end
		else
		{
			...
			UnitTypes eCaucasianUnit = (UnitTypes)GC.getCivilizationInfo(getCivilizationType()).getCivilizationUnits(GC.getDefineINT("DEFAULT_POPULATION_UNIT"));
			UnitTypes eAfricanSlaveUnit = (UnitTypes)GC.getCivilizationInfo(getCivilizationType()).getCivilizationUnits(GC.getDefineINT("UNITCLASS_AFRICAN_SLAVE"));
			UnitTypes eFreeAfricanUnit = (UnitTypes)GC.getCivilizationInfo(getCivilizationType()).getCivilizationUnits(GC.getDefineINT("UNITCLASS_FREED_SLAVE"));
			UnitTypes eNativeSlaveUnit = (UnitTypes)GC.getCivilizationInfo(getCivilizationType()).getCivilizationUnits(GC.getDefineINT("UNITCLASS_NATIVE_SLAVE"));
			UnitTypes eMestizoUnit = (UnitTypes)GC.getCivilizationInfo(getCivilizationType()).getCivilizationUnits(GC.getDefineINT("UNITCLASS_MESTIZO"));
			UnitTypes eFreeNativeUnit = (UnitTypes)GC.getCivilizationInfo(getCivilizationType()).getCivilizationUnits(GC.getDefineINT("UNITCLASS_CONVERTED_NATIVE"));
			int iRandomPopGrowth = GC.getGameINLINE().getSorenRandNum((int)m_aPopulationUnits.size(), "Diversify pop");

			for (uint i = 0; i < m_aPopulationUnits.size(); ++i)
			{
				if (i == (uint)iRandomPopGrowth)
				{
					if (m_aPopulationUnits[i]->getUnitType() == eAfricanSlaveUnit) { eUnit = eAfricanSlaveUnit; }
					else if (m_aPopulationUnits[i]->getUnitType() == eFreeAfricanUnit) { eUnit = eFreeAfricanUnit; }
					else if (m_aPopulationUnits[i]->getUnitType() == eNativeSlaveUnit) { eUnit = eNativeSlaveUnit; }
					else if (m_aPopulationUnits[i]->getUnitType() == eFreeNativeUnit) { eUnit = eFreeNativeUnit; }
					else if (m_aPopulationUnits[i]->getUnitType() == eMestizoUnit) { eUnit = eMestizoUnit; }
					else { eUnit = eCaucasianUnit; }
				}
			}
			if (eUnit == eNativeSlaveUnit || eUnit == eFreeNativeUnit)
			{
				if (iRandomPopGrowth % 2 == 0)
				{
					eUnit = eMestizoUnit;
				}
			}
	...
	else if (getFood() < 0)
	{
		...
		// Ramstormp, PTSD, Starve to DEATH
		std::vector<CvUnit*> aPopulationUnits;
		for (int iI = 0; iI < getPopulation(); iI++)
		{
			aPopulationUnits.push_back(getPopulationUnitByIndex(iI));
		}
...
//VET NewCapacity - end 4/9
	...
	for (YieldTypes eYield = FIRST_YIELD; eYield < NUM_YIELD_TYPES; ++eYield)
	{
		...
		default:
			...
			if (GC.getYieldInfo(eYield).isCargo() && eYield != YIELD_LUMBER && eYield != YIELD_STONE && calculateNetYield(eYield) > 0) // we do not sell YIELD_LUMBER and Stone to Overflow or Custom House
			{
...
const TCHAR* CvCity::getCityBillboardProductionIcon() const
{
	...
	if (!isHuman() && getOwnerINLINE() != GC.getGameINLINE().getActivePlayer() && AI_getDesiredYield() != NO_YIELD) // Ramstormp, PTSD, more dealings with other Europeans
	{
...
bool CvCity::getCityBillboardBottomBarValues(float& fStored, float& fRate, float& fRateExtra) const
{
	...
	if (isHuman() || getOwnerINLINE() == GC.getGameINLINE().getActivePlayer()) // Ramstormp, PTSD, more dealings with other Europeans
	{
...
GoodyTypes CvCity::getGoodyType(const CvUnit* pUnit) const
{
	...
	// Ramstormp, PTSD, Everyone gets goodies - START
	if (isHuman())
	{
...
{	
	...
	int iMinPopulation = GC.getMIN_POP_NEG_HAPPINESS(); //* 2; //Ramstormp, PTSD
...
{
	int iUnHapSlav = 0;
	// Ramstormp, PTSD, More gradual population based unhappiness from slaves - START
	int iPopulationCushion = getPopulation();
	for (int i = 0; i < getPopulation(); ++i)
	{
		if (iPopulationCushion - iUnHapSlav > 10)
		{
...
int CvCity::getUnhappinessFromMissingDefense() const
{
	...
	// Ramstormp, PTSD, no more weak defender happiness - START
	int iDefendersFound = 0;
	if (!isHuman())
	{
		iDefendersFound = plot()->getNumDefenders(getOwnerINLINE());
	}
	if (isHuman())
	{
...
int CvCity::getHappinessFromDomesticDemandsFulfilled() const
{
	...
	for (int iYield = 0; iYield < NUM_YIELD_TYPES; ++iYield)
	{
		...
		if (GC.getYieldInfo(eYield).isCargo())
		{
			// Ramstormp, PTSD, More happy from demands fulfilled - START
			if (getYieldDemand(eYield) > 1 && (getYieldStored(eYield) >= getYieldDemand(eYield)))
			{
				if (isHuman())
				{
					iHapDomesticDemand += 2; // getYieldDemand(eYield);
				}
				else
				{
					iHapDomesticDemand += 10;
				}
			}
			else if (getYieldDemand(eYield) > 0 && (getYieldStored(eYield) > 0))
			{
				if (isHuman())
				{
					iHapDomesticDemand += 1; // getYieldDemand(eYield) - getYieldStored(eYield);	
				}
				else
				{
					iHapDomesticDemand += 7;
				}
			}
			// Ramstormp - END	
		}
...
int CvCity::getHappinessFromTreaties() const
{
	...
	// Ramstormp, PTSD, less happy from treaties - START
	if (isHuman())
	{
		if (iNumOpenBorders > GC.getDefineINT("MAX_HAPPINESS_FROM_OPEN_BORDER_TREATIES")) // Ramstormp, PTSD, was commented out and min 5
		{
			iNumOpenBorders = GC.getDefineINT("MAX_HAPPINESS_FROM_OPEN_BORDER_TREATIES");
		}
		if (iNumDefensivePacts > GC.getDefineINT("MAX_HAPPINESS_FROM_DEFENSIVE_PACTS")) // Ramstormp, PTSD, was commented out and min 5
		{
			iNumDefensivePacts = GC.getDefineINT("MAX_HAPPINESS_FROM_DEFENSIVE_PACTS");
		}
	}
	// Ramstormp - END
