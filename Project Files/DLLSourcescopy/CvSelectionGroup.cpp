...
CvPlot* CvSelectionGroup::lastMissionPlot()
{
	...
	while (pMissionNode != NULL)
	{
		...
		case MISSION_BASIC_ROUTE_TO: // Ramstormp, WtP, Build Basic Road to Command
...
bool CvSelectionGroup::canStartMission(int iMission, int iData1, int iData2, CvPlot* pPlot, bool bTestVisible, bool bUseCache)
{
	...
	while (pUnitNode != NULL)
	{
		...
		case MISSION_ROUTE_TO:
			...
			break;
	// Ramstormp, WtP, 'Basic Road To' Command - START
		case MISSION_BASIC_ROUTE_TO:
			if (!(pPlot->at(iData1, iData2)) || (getCheapestBuildRoute(pPlot) != NO_ROUTE))
			{
				return true;
			}
			break;
		...
		if (getNumUnits() > 0)
		{
			if (canAllMove())
			{
				...
				if (!bDone)
				{
					...
					//Ramstormp, WtP, Build 'Basic Road To' Command - START 
					case MISSION_BASIC_ROUTE_TO: 
						if (groupBasicRoadTo(headMissionQueueNode()->m_data.iData1, headMissionQueueNode()->m_data.iData2, headMissionQueueNode()->m_data.iFlags))
						{
							bAction = true;
						}
						else
	...
	if ((getNumUnits() > 0) && (headMissionQueueNode() != NULL))
	{
		if (!bDone)
		{
			...
			case MISSION_ROUTE_TO:
				if (at(headMissionQueueNode()->m_data.iData1, headMissionQueueNode()->m_data.iData2))
				{
					if (getBestBuildRoute(plot()) == getCheapestBuildRoute(plot())) // Ramstormp, WtP, 'Basic Road to' first
					{
						bDone = true;
					}
				}
			...
			case MISSION_BASIC_ROUTE_TO:
				if (at(headMissionQueueNode()->m_data.iData1, headMissionQueueNode()->m_data.iData2))
				{
					if (getCheapestBuildRoute(plot()) == NO_ROUTE)
					{
						bDone = true;
					}
				}
				...
				if (getOwnerINLINE() == GC.getGameINLINE().getActivePlayer())
				{
					if (IsSelected())
					{
						if ((headMissionQueueNode()->m_data.eMissionType == MISSION_MOVE_TO) ||
							...
							(headMissionQueueNode()->m_data.eMissionType == MISSION_BASIC_ROUTE_TO) || // Ramstormp, WtP, Build 'Basic Road To' Command
...
bool CvSelectionGroup::canDoInterfaceMode(InterfaceModeTypes eInterfaceMode)
{
	...
	while (pUnitNode != NULL)
	{
		...
		case INTERFACEMODE_ROUTE_TO:
			...
			break;
			// Ramstormp, WtP, Build Basic 'Route to' first - START
		case INTERFACEMODE_BASIC_ROUTE_TO:
			if (pLoopUnit->workRate(true) > 0)
			{
				if (pLoopUnit->canBuildRoute())
				{
...
// Ramstormp, WtP, Build 'Basic Road To' First - START
RouteTypes CvSelectionGroup::getCheapestBuildRoute(CvPlot* pPlot, BuildTypes* peBestBuild) const
{
	PROFILE_FUNC();
	...
	iBestValue = GC.getNumBuildInfos();
	eCheapestRoute = NO_ROUTE;
	...
	while (pUnitNode != NULL)
	{
		...
		for (iI = 0; iI < GC.getNumBuildInfos(); iI++)
		{
			...
			if (eRoute != NO_ROUTE)
			{
				if ((pPlot->getRouteType() == NO_ROUTE) && pLoopUnit->canBuild(pPlot, ((BuildTypes)iI)))
				{
					iValue = GC.getRouteInfo(eRoute).getValue();
					if (iValue < iBestValue)
					{
						iBestValue = iValue;
						eCheapestRoute = eRoute;
						if (peBestBuild != NULL && eCheapestRoute == eRoute)
						{
							*peBestBuild = ((BuildTypes)iI);
						}
					}
...
bool CvSelectionGroup::groupRoadTo(int iX, int iY, int iFlags)
{
	...
	{
		pPlot = plot();
		if (isHuman() && (pPlot->getRouteType() == NO_ROUTE))
		{
			eBestRoute = getCheapestBuildRoute(pPlot, &eBestBuild);
		}
		else
		{
...
// Ramstormp, WtP, 'Basic Road To' Command - START
bool CvSelectionGroup::groupBasicRoadTo(int iX, int iY, int iFlags)
{
	CvPlot* pPlot;
	RouteTypes eCheapestRoute;
	BuildTypes eBestBuild;
	...
	if (!AI_isControlled() || !at(iX, iY) || (getLengthMissionQueue() == 1))
	{
		...
		eCheapestRoute = getCheapestBuildRoute(pPlot, &eBestBuild);
