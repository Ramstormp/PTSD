...
'''
#Ramstormp, PTSD, Harbor version of Large Rivers
def placeRiverFord(x,y):
    gc = CyGlobalContext()
    mmap = gc.getMap()
    terrainLargeRiver = gc.getInfoTypeForString("TERRAIN_LARGE_RIVERS")
    featureFord = gc.getInfoTypeForString("FEATURE_RIVER_FORD")
    fordPlot = mmap.plot(x,y)
    fordPlot.setFeatureType(featureFord,0)
    return
def makeLargeRiver(x,y):
    gc = CyGlobalContext()
    mmap = gc.getMap()
    terrainLargeRiver = gc.getInfoTypeForString("TERRAIN_LARGE_RIVERS")
    #featureFord = gc.getInfoTypeForString("FEATURE_RIVER_FORD")
    plot = mmap.plot(x,y)
    plotN = mmap.plot(x,y + 1)
    plotS = mmap.plot(x,y - 1)
    plotW = mmap.plot(x - 1,y)
    plotE = mmap.plot(x + 1,y)
#    fordPlot = mmap.plot(deltaX,deltaY)
    #if x > deltaX:
   # 	fordPlot = mmap.plot(x-2,y)
   # if x < deltaX:
   # 	fordPlot = mmap.plot(x+2,y)
   # if y > deltaY:
   # 	fordPlot = mmap.plot(x,y-2)
   # if y < deltaY:
   # 	fordPlot = mmap.plot(x,y+2)
    cleanUpLake(x,y)
    plot.setTerrainType(terrainLargeRiver,True,True)
    if x%2 == 0 and y%2 == 0:
	if plotN.isHills() == True and plotS.isHills() == True:
		placeRiverFord(x,y)
	elif plotE.isHills() == True and plotW.isHills() == True:
		placeRiverFord(x,y)
    plot.setRiverID(-1)
    #plot.setNOfRiver(False,CardinalDirectionTypes.NO_CARDINALDIRECTION)
    #plot.setWOfRiver(False,CardinalDirectionTypes.NO_CARDINALDIRECTION)
...
def makeHarbor(x,y,oceanMap):
    ...
    oceanMap.areaMap[ii] != oceanID:
        makeLargeRiver(x,y + 1)
	#placeRiverFord(xx,yy-1)
    ...
    oceanMap.getAreaByID(oceanMap.areaMap[ii]).water == True and \
    oceanMap.areaMap[ii] != oceanID:
        makeLargeRiver(x + 1,y)
	#placeRiverFord(xx-1,yy)
        oceanMap.defineAreas(isSmallWaterMatch)
        oceanID = oceanMap.getOceanID()
    #W
    xx = x - 2
...
def makeChannel(x,y):
    gc = CyGlobalContext()
    mmap = gc.getMap()
    terrainCoast = gc.getInfoTypeForString("TERRAIN_LAKE")
    plot = mmap.plot(x,y)
    cleanUpLake(x,y)
...
def expandLake(x,y,riversIntoLake,oceanMap):
    ...
    lakeSize = max(14,int(drainage * mc.LakeSizePerDrainage * desertModifier ))
...
#        lakeNeighbors.sort(key=operator.attrgetter('altitude'),reverse=False)
        ...
        if plot.isNOfRiver() == True or plot.isWOfRiver() == True:
            lakeSize -= 1
        makeLargeRiver(currentLakePlot.x,currentLakePlot.y)
#	if lakeSize == 2:
#		placeRiverFord(currentLakePlot.x,currentLakePlot.y)
