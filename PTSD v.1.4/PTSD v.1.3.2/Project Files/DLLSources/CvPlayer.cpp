...
void CvPlayer::initYieldStock()
{
	int iRandom, iEuropeStock, iAfricaStock, iPortRoyalStock;

	for (int iYield = 0; iYield < NUM_YIELD_TYPES; ++iYield)
	{
		YieldTypes eYield = (YieldTypes)iYield;
		iRandom = GC.getGameINLINE().getSorenRandNum(1000, "random Process") % 40 - 20;
		iEuropeStock = GC.getYieldInfo(eYield).getEuropeStock();
		iAfricaStock = GC.getYieldInfo(eYield).getAfricaStock();
...
void CvPlayer::processYieldStock()
{
	if (!isHuman())
	{
		return;
	...
	for (int iYield = 0; iYield < NUM_YIELD_TYPES; iYield++)
	{
		...
		// We want to fluctuate market price depends on stock and needs
		/*
		if (kYield.isRawMaterial())
		{
			processYieldEconomicTrend(eYield);
		}
		//else
		//{
			int iNewValue = getYieldRandomPercent(eYield) - 4 + GC.getGameINLINE().getSorenRandNum(9, "Update YieldRandomPercent");
			iNewValue = range(iNewValue, 80, 120);
			setYieldRandomPercent(eYield, iNewValue);
		//}*/
	}
}
// Ramstormp - END
...
CvUnit* CvPlayer::buyYieldUnitFromEurope(YieldTypes eYield, int iAmount, CvUnit* pTransport)
{
	...
	if (isHuman())
	{
		iAmount = std::min(iAmount, iEuropeStock);
	}
	if (iAmount == 0)
	{
...
void CvPlayer::doAction(PlayerActionTypes eAction, int iData1, int iData2, int iData3)
{
	...
	case PLAYER_ACTION_EUROPE_CHANGE_PROFESSION:
		...
		break;
// Ramstormp, PTSD, Change Profession in Africa Too - START
	case PLAYER_ACTION_AFRICA_CHANGE_PROFESSION:
		changeProfessionAfrica(iData1, (ProfessionTypes)iData2);
		break;
// Ramstormp - END
...
// Ramstormp, PTSD, Change professions in Africa too - START
void CvPlayer::changeProfessionAfrica(int iUnitId, ProfessionTypes eProfession)
{
	FAssert(getParent() != NO_PLAYER);
	CvUnit* pUnit = getAfricaUnitById(iUnitId);
	FAssert(pUnit != NULL);
	if (pUnit == NULL)
	{
		return;
	...
	if (eProfession != pUnit->getProfession())
	{
		...
		for (int i = 0; i < NUM_YIELD_TYPES; ++i)
		{
			YieldTypes eYieldType = (YieldTypes)i;
			// PTSD, Ramstormp, Europe Stock - START
			int iAfricaYieldStock = kEurope.getAfricaWarehouseYield(eYieldType);
			int iMissing = pUnit->getProfessionChangeYieldRequired(eProfession, eYieldType);
			if (iMissing > 0 && iMissing <= iAfricaYieldStock || !isHuman()) // after && added
			{
				iCost += iMissing * kEurope.getYieldAfricaSellPrice(eYieldType);
				changeYieldTradedTotal(eYieldType, iMissing);
				kEurope.changeYieldTradedTotal(eYieldType, iMissing);
				kEurope.changeAfricaWarehouseYield(eYieldType, -iMissing);
				GC.getGameINLINE().changeYieldBoughtTotal(kEurope.getID(), eYieldType, iMissing);
			}
			else if (iMissing > 0 && iMissing > iAfricaYieldStock)
			{
				return;
			}
			else if (iMissing < 0)
			{
				int iGold = getSellToAfricaProfit(eYieldType, -iMissing);
				iCost -= iGold;
				changeYieldTradedTotal(eYieldType, -iMissing);
				kEurope.changeYieldTradedTotal(eYieldType, -iMissing);
				kEurope.changeAfricaWarehouseYield(eYieldType, -iMissing);
