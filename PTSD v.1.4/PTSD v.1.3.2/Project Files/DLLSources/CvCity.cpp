...
void CvCity::doGrowth()
{
	...
	if (getFood() + getPopulation()/10 + getHappinessFromDomesticDemandsFulfilled()/10 >= growthThreshold()) // Ramstormp, PTSD, more growth
	{
	...
	else if (getFood() < 0)
	{
		...
		if (iDyingLot < aPopulationUnits.size() && pTargetUnit != NULL && !isNative())
		{
...
int CvCity::getHappinessFromTreaties() const
{
	...
	if (isHuman())
	{
		...
		{
			return GC.getDefineINT("MAX_HAPPINESS_FROM_OPEN_BORDER_TREATIES");
		}
		if (iNumDefensivePacts > GC.getDefineINT("MAX_HAPPINESS_FROM_DEFENSIVE_PACTS")) // Ramstormp, PTSD, was commented out and min 5
		{
			return GC.getDefineINT("MAX_HAPPINESS_FROM_DEFENSIVE_PACTS");
		}
	}
	// Ramstormp - END
...
//Ramstomrp, PTSD, totally unnecessary function at the moment - START
int CvCity::getTotalDomesticDemands() const
{
	int iTotalDomesticDemands = 0;
	// loop the Yields and find demand fulfilled
	for (int iYield = 0; iYield < NUM_YIELD_TYPES; ++iYield)
	{
		YieldTypes eYield = (YieldTypes)iYield;
		if (GC.getYieldInfo(eYield).isCargo())
		{
			iTotalDomesticDemands += getYieldDemand(eYield);
		}
	}
	return iTotalDomesticDemands;
}
// Ramstormp - END
