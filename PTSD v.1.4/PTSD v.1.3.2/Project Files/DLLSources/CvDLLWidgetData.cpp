...
bool CvDLLWidgetData::executeDoubleClick(const CvWidgetDataStruct& widgetDataStruct)
{
	...
	/*** TRIANGLETRADE 10/28/08 by DPII ***/
	case WIDGET_DOCK_AFRICA:
		doDoubleClickAfrica(widgetDataStruct);
...
void CvDLLWidgetData::doDoubleClickAfrica(const CvWidgetDataStruct& widgetDataStruct)
{
	CvUnit* pUnit = GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getAfricaUnitById(widgetDataStruct.m_iData1);
	if (pUnit != NULL)
	{
		CvPopupInfo* pInfo = new CvPopupInfo(BUTTONPOPUP_CHOOSE_PROFESSION, -1, pUnit->getID(), 0);
		gDLL->getInterfaceIFace()->addPopup(pInfo, NO_PLAYER, true);
