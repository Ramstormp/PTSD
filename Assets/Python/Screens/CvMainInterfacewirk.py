...
BUILDING_DATA[0] = [30, 22, 21, 12] #Bells
BUILDING_DATA[1] = [44, 6, 18, 10] #Courthouse
BUILDING_DATA[2] = [44, 22, 22, 12] #Hammers
BUILDING_DATA[3] = [58, 18, 20, 12] #Meat
BUILDING_DATA[4] = [30, 40, 20, 12] #Coats
BUILDING_DATA[5] = [44, 40, 21, 12] #Leather
BUILDING_DATA[6] = [30, 6, 17, 10] #Cloth
BUILDING_DATA[7] = [86, 56, 20, 12]  #Coloured Cloth
BUILDING_DATA[8] = [16, 42, 22, 12] #Beer
BUILDING_DATA[9] = [30, 76, 20, 12] #Rum
BUILDING_DATA[10] = [58, 57, 20, 12] #Wine
BUILDING_DATA[11] = [16, 26, 19, 12] #Cigars
BUILDING_DATA[12] = [16, 82, 22, 13] #Whale oil
BUILDING_DATA[13] = [30, 58, 20, 12] #Furniture
BUILDING_DATA[14] = [2, 10, 22, 10] #Tools
BUILDING_DATA[15] = [58, 35, 24, 12] #Crosses
BUILDING_DATA[16] = [2, 30, 20, 12] #Muskets
BUILDING_DATA[17] = [72, 74, 20, 10] #Fireworker
BUILDING_DATA[18] = [70, 4, 17, 9] #Horses
BUILDING_DATA[19] = [4, 86, 17, 10] #Dock
BUILDING_DATA[20] = [58, 3, 16, 10] #WareHouse
BUILDING_DATA[21] = [2, 48, 20, 12] #Spices
BUILDING_DATA[22] = [44, 76, 21, 12] #Domestic Market
BUILDING_DATA[23] = [16, 6, 18, 9] #Tavern
BUILDING_DATA[24] = [86, 36, 23, 12] #Education
BUILDING_DATA[25] = [87, 78, 18, 9] #Wall
BUILDING_DATA[26] = [6, 3, 12, 6] #Print
BUILDING_DATA[27] = [90, 2, 10, 5] #Mill
BUILDING_DATA[28] = [85, 8, 18, 9] #Trainstation
BUILDING_DATA[29] = [82, 3, 12, 6] #Corn
BUILDING_DATA[30] = [72, 35, 20, 12] #Medical Care
BUILDING_DATA[31] = [96, 8, 12, 6] #Well
BUILDING_DATA[32] = [84, 73, 20, 10] #Defense
BUILDING_DATA[33] = [44, 58, 21, 12] #Confectioner
BUILDING_DATA[34] = [16, 62, 20, 12] #Leather Padder
BUILDING_DATA[35] = [2, 65, 20, 12] #Cooking Oil Trader
BUILDING_DATA[36] = [58, 76, 22, 12] #Peanut Roaster
BUILDING_DATA[37] = [72, 56, 20, 12] #Potter
BUILDING_DATA[38] = [72, 17, 20, 11] #Tailor
BUILDING_DATA[39] = [86, 17, 14, 10] #Baker
BUILDING_DATA[40] = [54, 6, 12, 8] #Statue
BUILDING_DATA[41] = [34, 3, 12, 6] #Graveyard
BUILDING_DATA[42] = [0, 7, 10, 5] #Jail
...
#PTSD
		screen.addPanel("CityFullBackground", u"", u"", True, False, 0, CITY_TITLE_BAR_HEIGHT, xResolution, yResolution, PanelStyles.PANEL_STYLE_STANDARD, WidgetTypes.WIDGET_GENERAL, -1, -1 )
		screen.addDrawControl("CityFullBackground", ArtFileMgr.getInterfaceArtInfo("INTERFACE_GRASS_BG").getPath(), 0, CITY_TITLE_BAR_HEIGHT, xResolution, yResolution, WidgetTypes.WIDGET_GENERAL, -1, -1 )
		self.appendtoHideState(screen, "CityFullBackground", HIDE_TYPE_CITY, HIDE_LEVEL_ALL)
	...
	# RESOURCE TABLE
		...
		for iYield in self.TableYields:
			...
			screen.setLabel("YieldStoredShadowlabel" + str(i), "ResourceTable", "", CvUtil.FONT_CENTER_JUSTIFY, iX, iZ, -0.3, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_RECEIVE_MOVE_CARGO_TO_CITY, -1, -1 )
			screen.setLabel("YieldStoredlabel" + str(iYield), "ResourceTable", "", CvUtil.FONT_CENTER_JUSTIFY, iX, iZ, -0.3, FontTypes.GAME_FONT, WidgetTypes.WIDGET_RECEIVE_MOVE_CARGO_TO_CITY, -1, -1 )
			iZ = yResolution - (STACK_BAR_HEIGHT * 2) / iK1
			screen.setLabel("YieldNetShadowlabel" + str(i), "ResourceTable", "", CvUtil.FONT_CENTER_JUSTIFY, iX, iZ, -0.3, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_RECEIVE_MOVE_CARGO_TO_CITY, -1, -1 )
			screen.setLabel("YieldNetlabel" + str(iYield), "ResourceTable", "", CvUtil.FONT_CENTER_JUSTIFY, iX, iZ, -0.3, FontTypes.GAME_FONT, WidgetTypes.WIDGET_RECEIVE_MOVE_CARGO_TO_CITY, -1, -1 )

			iX = STACK_BAR_HEIGHT + ((iYield - iDelta) * RESOURCE_TABLE_COLUMN_WIDTH) + (RESOURCE_TABLE_COLUMN_WIDTH - self.iIconWidth) / 2 + xDelta

			screen.addDragableButton("YieldIcon" + str(iYield), gc.getYieldInfo(iYield).getIcon(), "", iX, iY * 99/98, self.iIconWidth* 4/5, self.iIconWidth* 4/5, WidgetTypes.WIDGET_MOVE_CARGO_TO_TRANSPORT, iYield, -1, ButtonStyles.BUTTON_STYLE_IMAGE )
...
#<--- TAC
	...
	# Garrison and Transport Panel
		screen.addScrollPanel("CityGarrisonPanel", u"", CITIZEN_BAR_WIDTH + (SMALL_BUTTON_SIZE / 8), yResolution - BOTTOM_CENTER_HUD_HEIGHT - TRANSPORT_AREA_HEIGHT * 9 / 8 - 26, xResolution - CITIZEN_BAR_WIDTH - TRANSPORT_AREA_WIDTH + STACK_BAR_HEIGHT * 4 / 8, TRANSPORT_AREA_HEIGHT - (STACK_BAR_HEIGHT / 3), PanelStyles.PANEL_STYLE_EMPTY, false, WidgetTypes.WIDGET_EJECT_CITIZEN, -1, -1 )	
...
###=====City Radius 2 (NeverMind)===2/2=====###
		...
		#					screen.addDDSGFC("CityPlotInfo" + str(iPlotIndex), "", x-(size_x/32), y-(size_y/4), size_x, size_y/4, #WidgetTypes.WIDGET_CITY_PLOT_INFO, iPlotIndex, -1) 
		#					CitizenHideList.append("CityPlotInfo" + str(iPlotIndex))
							...
							if (not pUnit.isNone()):
								if (pUnit.isColonistLocked()):
									...
									screen.addDDSGFC(szName, ArtFileMgr.getInterfaceArtInfo("INTERFACE_CITY_CITIZEN_LOCK_GLOW").getPath(), int(screenPosition.x)+ButtonSize/24, int(screenPosition.y)+ButtonSize*2/8, ButtonSize/2, ButtonSize/3, WidgetTypes.WIDGET_GENERAL, -1, -1)
						...
						if (gc.getBuildingInfo(CityBuilding).getMaxWorkers() >= 0):
							...
							if (CityBuilding != -1 and iProfession != -1):
								...
								for GroupIndex in range(ProfessionCount):
									...
									if (pCitizen.isColonistLocked()):
										szName = "CitizenButtonLock" + str(pCitizen.getID())
										screen.addDDSGFC(szName, ArtFileMgr.getInterfaceArtInfo("INTERFACE_CITY_CITIZEN_LOCK_GLOW").getPath(), BUILDING_GRID[iSpecialBuildingType][0] + (CitizenSpacing * GroupIndex) + (CitizenSpacing / 2) , BUILDING_GRID[iSpecialBuildingType][1] + BUILDING_GRID[iSpecialBuildingType][2] - STACK_BAR_HEIGHT, ButtonSize/2, ButtonSize/4, WidgetTypes.WIDGET_GENERAL, -1, -1)
										CitizenHideList.append(szName)
		...
		for i in range(CyInterface().getNumCachedInterfacePlotUnits()):
			...
			if (pLoopUnit and pLoopUnit.getOwner() == pHeadSelectedCity.getOwner()):
				if (pLoopUnit.cargoSpace() > 0 and not pLoopUnit.isCargo()):
					...
					if (gc.getUnitInfo(pLoopUnit.getUnitType()).getDomainType() == DomainTypes.DOMAIN_SEA):
						screen.addCheckBoxGFCAt("CityTransportPanel", "VisitingShipIcon" + str(i), pLoopUnit.getFullLengthIcon(), "", xPosition, yLocation - (TransportButtonSize / 2), TransportButtonSize*2, TransportButtonSize*3, WidgetTypes.WIDGET_RECEIVE_MOVE_CARGO_TO_TRANSPORT, pLoopUnit.getID(), -1, ButtonStyles.BUTTON_STYLE_LABEL)
						xPosition += TransportButtonSize * 5/3
					
					else:
						screen.addCheckBoxGFCAt("CityTransportPanel", "VisitingShipIcon" + str(i), pLoopUnit.getButton(), "", xPosition,yLocation + (PanelHeight / 2) - (TransportButtonSize / 2), TransportButtonSize*2/3, TransportButtonSize*2/3, WidgetTypes.WIDGET_RECEIVE_MOVE_CARGO_TO_TRANSPORT, pLoopUnit.getID(), -1, ButtonStyles.BUTTON_STYLE_LABEL)
						xPosition += TransportButtonSize * 2/3					
					...
					for j in range (iCargoSpace):
						if (pLoopUnit.isTroopShip()): 
							screen.addDDSGFCAt("CargoCell" + str(i) + "-" + str(j), "CityTransportPanel", ArtFileMgr.getInterfaceArtInfo("INTERFACE_EUROPE_BOX_CARGO_TROOPS").getPath(), xPosition + (j * (CargoButtonSize * 11 / 10)), yLocation - (CargoButtonSize / 4), CargoButtonSize, CargoButtonSize, WidgetTypes.WIDGET_RECEIVE_MOVE_CARGO_TO_TRANSPORT, pLoopUnit.getID(), -1, False)
						elif (pLoopUnit.isTreasureShip()):
							if(j%3 == 1): 
								screen.addDDSGFCAt("CargoCell" + str(i) + "-" + str(j), "CityTransportPanel", ArtFileMgr.getInterfaceArtInfo("INTERFACE_EUROPE_BOX_CARGO_TREASURE_M").getPath(), xPosition + (j * (CargoButtonSize * 11 / 10)), yLocation - (CargoButtonSize / 4), CargoButtonSize, CargoButtonSize, WidgetTypes.WIDGET_RECEIVE_MOVE_CARGO_TO_TRANSPORT, pLoopUnit.getID(), -1, False)
							elif(j%3 == 2):
								screen.addDDSGFCAt("CargoCell" + str(i) + "-" + str(j), "CityTransportPanel", ArtFileMgr.getInterfaceArtInfo("INTERFACE_EUROPE_BOX_CARGO_TREASURE_R").getPath(), xPosition + (j * (CargoButtonSize * 11 / 10)), yLocation - (CargoButtonSize / 4), CargoButtonSize, CargoButtonSize, WidgetTypes.WIDGET_RECEIVE_MOVE_CARGO_TO_TRANSPORT, pLoopUnit.getID(), -1, False)
							else:
								screen.addDDSGFCAt("CargoCell" + str(i) + "-" + str(j), "CityTransportPanel", ArtFileMgr.getInterfaceArtInfo("INTERFACE_EUROPE_BOX_CARGO_TREASURE_L").getPath(), xPosition + (j * (CargoButtonSize * 11 / 10)), yLocation - (CargoButtonSize / 4), CargoButtonSize, CargoButtonSize, WidgetTypes.WIDGET_RECEIVE_MOVE_CARGO_TO_TRANSPORT, pLoopUnit.getID(), -1, False)
						elif (pLoopUnit.specialCargo() != SpecialUnitTypes.NO_SPECIALUNIT):
						...
						elif (j >= (iCargoSpace / 2 + iCargoSpace % 2) or pLoopUnit.isSlaveShip()):
							screen.addDDSGFCAt("CargoCell" + str(i) + "-" + str(j), "CityTransportPanel", ArtFileMgr.getInterfaceArtInfo("INTERFACE_EUROPE_BOX_CARGO_HOLD").getPath(), xPosition + (j * (CargoButtonSize * 11 / 10)), yLocation - (CargoButtonSize / 4), CargoButtonSize, CargoButtonSize, WidgetTypes.WIDGET_RECEIVE_MOVE_CARGO_TO_TRANSPORT, pLoopUnit.getID(), -1, False)
						else:
							screen.addDDSGFCAt("CargoCell" + str(i) + "-" + str(j), "CityTransportPanel",ArtFileMgr.getInterfaceArtInfo("INTERFACE_EUROPE_BOX_CARGO_GOODS").getPath(), xPosition + (j * (CargoButtonSize * 11 / 10)), yLocation - (CargoButtonSize / 4), CargoButtonSize, CargoButtonSize, WidgetTypes.WIDGET_RECEIVE_MOVE_CARGO_TO_TRANSPORT, pLoopUnit.getID(), -1, False)
					...
					for j in range(CyInterface().getNumCachedInterfacePlotUnits()):
						...
						if (not transportUnit.isNone() and transportUnit.getID() == pLoopUnit.getID()):
							...
							if CargoUnit.isGoods():
								...
								if (not gc.getUnitInfo(CargoUnit.getUnitType()).isTreasure()):
									xTreasureSpacing = 0
							...
							xPosition += (CargoButtonSize * 11 / 10) + xTreasureSpacing
							if (gc.getUnitInfo(CargoUnit.getUnitType()).isTreasure()):
								for iI in range (2): # todo getBerthSize()
#								j +=1
									screen.addDragableButtonAt("CityTransportPanel", "GoldPile" + str(iI), ArtFileMgr.getInterfaceArtInfo("SCREEN_GOLD_PILE").getPath(), "", xPosition, yLocation - (CargoButtonSize / 4), CargoButtonSize, CargoButtonSize, WidgetTypes.WIDGET_MOVE_CARGO_TO_CITY, pLoopUnit.getID(), CargoUnit.getID(), ButtonStyles.BUTTON_STYLE_LABEL)
									xPosition += (CargoButtonSize * 11 / 10)
#								j +=1
#								screen.addDragableButtonAt("CityTransportPanel", "CargoUnit" + str(j), ArtFileMgr.getInterfaceArtInfo("SCREEN_GOLD_PILE").getPath(), "", xPosition, yLocation - (CargoButtonSize / 4), CargoButtonSize, CargoButtonSize, WidgetTypes.WIDGET_MOVE_CARGO_TO_CITY, pLoopUnit.getID(), CargoUnit.getID(), ButtonStyles.BUTTON_STYLE_LABEL)
#								xPosition += (CargoButtonSize * 11 / 10)
							else:
								xTreasureSpacing = 0
					#xPosition += (CargoButtonSize * 11 / 10) + xTreasureSpacing
	...
	# GARISSON MANAGMENT PANEL
		...
		xPositionBase = (xResolution - CITIZEN_BAR_WIDTH - TRANSPORT_AREA_WIDTH * 11/10 - STACK_BAR_HEIGHT - GarrisonUnitsWidth) / 2
		iXUp = xPositionBase 
		iXDown = xPositionBase
		iNumGarrisoned = CyInterface().getNumCachedInterfacePlotUnits()
		if (iNumGarrisoned < 3):
			iNumGarrisoned = 3 
		yPosition = 1/2
		for i in range(CyInterface().getNumCachedInterfacePlotUnits()):
...
#<--- TAC
							cityGarrisonY = STACK_BAR_HEIGHT * 90/44 + (yPosition * (GarrisonButtonSize + self.PLOT_LIST_HEALTH_BAR_SHORTENING_CORRECTION))
							if (pLoopUnit.baseCombatStr() > 2 and pLoopUnit.baseMoves() < 2):
								screen.addDragableButtonAt("CityGarrisonPanel", "GarrisonUnit" + str(i), pLoopUnit.getFullLengthIcon(), "", iXUp, cityGarrisonY - ButtonSize*10/22, ButtonSize/2, ButtonSize, WidgetTypes.WIDGET_EJECT_CITIZEN, pLoopUnit.getID(), -1, ButtonStyles.BUTTON_STYLE_LABEL)
								...
								if (pLoopUnit.canMove()):
									...
									szFileName = ArtFileMgr.getInterfaceArtInfo("OVERLAY_NOMOVE").getPath()
									#screen.addDDSGFCAt( "GarrisonMoveIcon" + str(i), "CityGarrisonPanel", szFileName, iXUp, (STACK_BAR_HEIGHT * 3 / 2) + cityGarrisonY - ButtonSize*15/22, self.PLOT_LIST_ICON_SIZE, self.PLOT_LIST_ICON_SIZE, WidgetTypes.WIDGET_EJECT_CITIZEN, -1, -1, False )										
							elif (not gc.getUnitInfo(pLoopUnit.getUnitType()).isTreasure()):
								#Ramstormp, PTSD, well, where do I hide the treasure?
								screen.addDragableButtonAt("CityGarrisonPanel", "GarrisonUnit" + str(i), pLoopUnit.getFullLengthIcon(), "", iXDown, cityGarrisonY + ButtonSize * 1/4, ButtonSize/2, ButtonSize, WidgetTypes.WIDGET_EJECT_CITIZEN, pLoopUnit.getID(), -1, ButtonStyles.BUTTON_STYLE_LABEL)
								if pLoopUnit.isWaiting():
									szFileName = ArtFileMgr.getInterfaceArtInfo("OVERLAY_FORTIFY").getPath()
								if (pLoopUnit.canMove()):
									if (pLoopUnit.hasMoved()):
									...
									szFileName = ArtFileMgr.getInterfaceArtInfo("OVERLAY_NOMOVE").getPath()
									screen.addDDSGFCAt( "GarrisonMoveIcon" + str(i), "CityGarrisonPanel", szFileName, iXDown, cityGarrisonY + ButtonSize * 1/4, self.PLOT_LIST_ICON_SIZE, self.PLOT_LIST_ICON_SIZE, WidgetTypes.WIDGET_EJECT_CITIZEN, -1, -1, False )	
							else:#if (not pLoopUnit.canKingTransport()):
							#Ramstormp, PTSD, well, where do I hide the treasure?
								screen.addDragableButtonAt("CityGarrisonPanel", "GarrisonUnit" + str(i), pLoopUnit.getFullLengthIcon(), "", iXDown, cityGarrisonY + ButtonSize * 1/8, ButtonSize/2, ButtonSize, WidgetTypes.WIDGET_EJECT_CITIZEN, pLoopUnit.getID(), -1, ButtonStyles.BUTTON_STYLE_LABEL)
								if pLoopUnit.isWaiting():
									szFileName = ArtFileMgr.getInterfaceArtInfo("OVERLAY_FORTIFY").getPath()
								if (pLoopUnit.canMove()):
									if (pLoopUnit.hasMoved()):
						...
						# HEALTH BAR
							...
							iXUp += (ButtonSize/11) * 12 / iNumGarrisoned
							iXDown += (ButtonSize/11) * 12 / iNumGarrisoned
							#xPosition += (GarrisonButtonSize / 2) * (9 / iNumGarrisonUnits)
							#iNumGarrisonUnits += 1
							#xPosition += 1
							#if (xPosition == 3):
							#	xPosition = 0
...
#VET NewCapacity - end 1/1
			...
			if ( CyInterface().getShowInterface() == InterfaceVisibility.INTERFACE_SHOW ):
				...
				for iYield in range(YieldTypes.NUM_YIELD_TYPES):
					...
					screen.hide("YieldStoredShadowlabel" + str(iYield))
...
# VET DynamicYieldsIcon - 13/15 - end
				...
				iDemanded = pCity.getYieldDemand(i)
				...
				if ((iStored > 0 and iDemanded > 0 and iRate >= iDemanded) or iStored > 0 or iRate > 0):
					if (iStored > pCity.getMaxYieldCapacity() and not bIgnoredForStorageCapacity):
						szStoredShadow = u"<color=25,55,25>%d</color>" %(iStored)
						szStored = u"<color=255,0,0>%d</color>" %(iStored)
					else:
						szStoredShadow = u"<color=25,25,25>%d</color>" %(iStored)	
						szStored = u"<color=100,255,255>%d</color>" %(iStored)
				elif (iStored == 0):
					szStoredShadow = u""				
					szStored = u""
				else:
					szStoredShadow = u"<color=25,25,25>%d</color>" %(iStored)	
					szStored = u"<color=255,100,90>%d</color>" %(iStored)
				szRate = u"%d" %(iRate)
				szRateShadow = u"%d" %(iRate)
				if(iRate > 0):
					szRate = u"<color=0,255,0>+" + szRate + u"</color>"
					szRateShadow = u"<color=35,25,55>+" + szRateShadow + u"</color>"
				elif(iRate == 0):
					szRate = u""
					szRateShadow = u""
				else:
					szRate = u"<color=255,255,100>" + szRate + u"</color>"
					szRateShadow = u"<color=35,25,55>" + szRateShadow + u"</color>"
				szDemanded = u"%d" %(iDemanded)
				szDemandedShadow = u"%d" %(iDemanded)
				if(iDemanded > 0 and iStored == 0):
					szDemanded =  u"<color=125,145,139>%d</color>" %(iDemanded)
					szDemandedShadow = u"<color=35,25,55>+" + szDemandedShadow + u"</color>"
				elif(iDemanded == 0):
					szDemanded = u""
					szDemandedShadow = u""
				elif(iDemanded > 0 and iStored > 0):
					szDemanded = u"<color=255,240,100>-" + szDemanded + u"</color>"
					szDemandedShadow = u"<color=35,25,55>-" + szDemandedShadow + u"</color>"
				szStorageLabel = "YieldStoredlabel" + str(i)
				szStorageShadowLabel = "YieldStoredShadowlabel" + str(i)

				iX = STACK_BAR_HEIGHT + ((index - iDelta) * RESOURCE_TABLE_COLUMN_WIDTH) + (RESOURCE_TABLE_COLUMN_WIDTH * 19/25) + xDelta
				iY = yResolution - BOTTOM_CENTER_HUD_HEIGHT / iK1 + self.iIconWidth - yResolution / 18
				# WTP, ray, 3 rows
				if (iK1 == 2):
					iY = yResolution - (2 * BOTTOM_CENTER_HUD_HEIGHT / 3) + self.iIconWidth - yResolution / 18
				elif (iK1 == 3):
					iY = yResolution - BOTTOM_CENTER_HUD_HEIGHT / 3 + self.iIconWidth - yResolution / 18
				screen.setLabel(szStorageShadowLabel, "", self.setFontSize(szStoredShadow, 0), CvUtil.FONT_CENTER_JUSTIFY, iX-41/20, iY-41/20, -0.3, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_RECEIVE_MOVE_CARGO_TO_CITY, -1, -1 )
				screen.setLabel(szStorageLabel, "", self.setFontSize(szStored, 0), CvUtil.FONT_CENTER_JUSTIFY, iX, iY, -0.3, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_RECEIVE_MOVE_CARGO_TO_CITY, -1, -1 )
# VET DynamicYieldsIcon - 14/15 - start
				ResourceHideList.append(szStorageShadowLabel)
				ResourceHideList.append(szStorageLabel)
				szRateLabel = "YieldNetlabel" + str(i)
				szRateShadowLabel = "YieldNetShadowlabel" + str(i)
# VET DynamicYieldsIcon - 15/15 - start
				...
				iY += yDelta
				screen.setLabel(szRateShadowLabel, "", self.setFontSize(szRateShadow, 0), CvUtil.FONT_CENTER_JUSTIFY, iX-41/20, iY-41/20, -0.3, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_RECEIVE_MOVE_CARGO_TO_CITY, -1, -1 )
...
# VET DynamicYieldsIcon - 15/15 - start
				...
				ResourceHideList.append(szRateShadowLabel)
			
				szDemandedLabel = "YieldDemandedlabel" + str(i)
				szDemandedShadowLabel = "YieldDemandedShadowlabel" + str(i)
				iY += yDelta
				screen.setLabel(szDemandedShadowLabel, "", self.setFontSize(szDemandedShadow, 0), CvUtil.FONT_CENTER_JUSTIFY, iX-41/20, iY-31/20, -0.3, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_RECEIVE_MOVE_CARGO_TO_CITY, -1, -1 )
				screen.setLabel(szDemandedLabel, "", self.setFontSize(szDemanded, 0), CvUtil.FONT_CENTER_JUSTIFY, iX, iY, -0.3, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_RECEIVE_MOVE_CARGO_TO_CITY, -1, -1 )
				ResourceHideList.append(szDemandedLabel)
				ResourceHideList.append(szDemandedShadowLabel)
